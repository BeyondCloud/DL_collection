...
# image display related function
from matplotlib import pyplot as plt
import matplotlib.image as mpimg

def loadImage(filePath ,filename):
    imagePath = os.path.join(filePath, filename)
    return imread(imagePath)

# show original real image
def showImage(content, style):
    fig, axs = plt.subplots(1, 2, figsize=(30,30))
    axs[0].imshow(content/255)
    axs[0].set_title('Content')
    axs[1].imshow(style/255)
    axs[1].set_title('Style')
        
def showResult(content, style, output):
    fig, axs = plt.subplots(1, 3, figsize=(18, 18))
    axs[0].imshow(content/255)
    axs[0].set_title('Content')
    axs[1].imshow(mpimg.imread(output))
    axs[1].set_title('Generated image')
    axs[2].imshow(style/255)
    axs[2].set_title('Style')
    
def showResultwMultiStyles(content, styles, output):
    fig, axs = plt.subplots(1, 2+len(styles), figsize=(18, 18))
    axs[0].imshow(content/255)
    axs[0].set_title('Content')
    axs[1].imshow(mpimg.imread(output))
    axs[1].set_title('Generated image')
    for i in range(len(styles)):
        axs[i+2].imshow(styles[i]/255)
        axs[i+2].set_title('Style '+str(i))

def showResults(outputs, ratios):
    fig, axs = plt.subplots(2, 3, figsize=(15, 15))
    for i in range(len(outputs)):
        axs[i//3, i%3].imshow(mpimg.imread(outputs[i]))
        axs[i//3, i%3].set_title(ratios[i])
